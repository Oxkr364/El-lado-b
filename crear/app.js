...
function renderPreview(result) {
  ...
  $('preview').hidden = false; $('saveTitle').textContent = 'Tu historia no fue publicada.'; $('saveDetail').textContent = currentUser ? 'Puedes guardarla ahora como borrador privado.' : 'Inicia sesión para guardarla de forma segura.'; $('saveBtn').textContent = currentUser ? 'GUARDAR BORRADOR PRIVADO' : 'INICIAR SESIÓN PARA GUARDAR'; $('saveBtn').disabled = false; $('preview').scrollIntoView({ behavior: 'smooth', block: 'start' });
}
...
async function persistStory() {
  ...
  try {
    ...
    $('saveTitle').textContent = 'Borrador guardado de forma segura.'; $('saveDetail').textContent = `Identificador privado: ${data.slug}`; button.textContent = 'BORRADOR GUARDADO ✓'; localStorage.removeItem('plan_b_creator_draft'); await loadSavedStories();
